...
if __name__ == '__main__':
    ...
    dfrll = pd.DataFrame(
        np.transpose(astr.cart2sph(np.vstack([df_xin.values.ravel(), df_yin.values.ravel(), df_zin.values.ravel()]).T)),
        columns=['geoc_r', 'geoc_lat', 'geoc_lon'])
    # print(dfrll.dropna())
    # dfrll.geoc_r.plot()
    # plt.show()
    # exit()
    ...
    path_tab = XovOpt.get("rawdir") + 'SIM_' + args[:2] + '/' + args + '/0res_*amp/'
    dirs = glob.glob(path_tab)
    files = glob.glob(f"{path_tab}{XovOpt.get('instrument')}*.TAB")

    ncols = len(dirs)
    try:
        assert len(files) > 0
    except:
        logging.error(f"*** No files found in {path_tab}. Exit.")
    ...
    for f in files:
        ...
        # vecopts = {}
        track = gtrack(opts=XovOpt.to_dict())
        # print(f)
        track.read_fill(f)
        tracks.append(track)
    ...
    for idx, track in tqdm(enumerate(tracks), total=len(tracks)):
        df = track.ladata_df[['ET_TX', 'geoc_long', 'geoc_lat', 'altitude', 'seqid']]
        tmp_seqid = df.loc[:, 'seqid'].copy()
        df['seqid'] = tmp_seqid.values + 1
        tmp_altitude = df.loc[:, 'altitude'].copy()
        df['altitude'] = 1737.4e3 + tmp_altitude.values
        dfxyz = pd.DataFrame(
            np.transpose(astr.sph2cart(df['altitude'].values, df['geoc_lat'].values, df['geoc_long'].values)),
            columns=['geoc_x', 'geoc_y', 'geoc_z'])
    ...
    logging.info(f'- Done reading {len(tracks)}, dumping info to output files...')
    ...
    (dflat - pd.DataFrame(df_latin)).to_csv(outdir_ + f'/{args}_out.dlat', header=None, index=None, sep='\t',
                                            na_rep='Nan')
    (dflon - pd.DataFrame(df_lonin)).to_csv(outdir_ + f'/{args}_out.dlon', header=None, index=None, sep='\t',
                                            na_rep='Nan')
    (dfelv - pd.DataFrame(df_rin * 1.e3)).to_csv(outdir_ + f'/{args}_out.dr', header=None, index=None, sep='\t',
                                                 na_rep='Nan')
