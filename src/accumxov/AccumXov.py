...
def prepro_weights_constr(xovi_amat, previous_iter=None):
    ...
    if AccOpt.get("use_advanced_weighting"):
        ...
        if previous_iter != None and previous_iter.converged:
            ...
            if XovOpt.get("debug") and not AccOpt.get("remove_max_dist") and not AccOpt.get(
                    "remove_3sigma_median") and not AccOpt.get("remove_dR200"):
            ...
            if XovOpt.get("debug") and XovOpt.get("local"):
                ...
                num_bins = 200  # 'auto'
                n, bins, patches = plt.hist(tmp, bins=num_bins,
                                            range=[1.e-4, 4.e-2])  # , cumulative=-1) #, density=True, facecolor='blue',
    ...
    if XovOpt.get("debug") and XovOpt.get("local"):  # len(sol4_pars) < 50 and debug:
        ...
        for idx, mat in enumerate([BTB.todense(), N_proj]):
            ...
            for i in range(5):
                print("Norm of Vh", np.round(np.linalg.norm(Vh.T[:, :i + 1], axis=1) * 100., 1), "% up to lambda= ",
                      S[i])
        # exit()
    ...
    for constrain in par_constr.items():
        ...
        if not AccOpt.get("remove_max_dist") and not AccOpt.get("remove_3sigma_median") and not AccOpt.get(
            ...
            n_goodobs_tracks = xovi_amat.xov.xovers.loc[xovi_amat.weights.diagonal() > 0.1 * weights_mean][
                ['orbA', 'orbB']].apply(pd.Series.value_counts).sum(axis=1).sort_values(
                ascending=False)
        else:
        ...
        csr.append(
            csr_matrix((np.power(AccOpt.get("sigma_0") / val, 2), (row, col)), dtype=np.float32,
                       shape=(len(sol4_pars), len(sol4_pars))))
        ...
        if XovOpt.get("partials"):
            ...
            for i in (i for i in range(10) if keep_iterating_vce):
                ...
                Q = np.linalg.cholesky(
                    (weight_constr * xovi_amat.penalty_mat + weight_constr_avg * xovi_amat.penalty_mat_avg).todense())
                ...
                if AccOpt.get("compute_vce"):
                    ...
                    if keep_iterating_vce:
                        ...
                        print("w_obs updated by", (np.abs(weight_obs - 1. / sigma2_obs) / weight_obs * 100.),
                              '% and w_constr by',
