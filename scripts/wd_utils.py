...
def xov_pkl2csv2(pyout_folder, csv_filename, arc1, arc2, ids, xov_time = True,  distance = True, derivatives = True):
    import numpy as np
    import pickle
    import csv
    import os.path
    ...
    file = open(csv_filename,'w')
    xov_header_cvs(file, xov_time,  distance, derivatives)

    writer = csv.writer(file, delimiter='\t')

    for id in ids:
      in_folder = f"{pyout_folder}{id}/"
      # in_folder = f"{in_folder}3res_20amp/"

      xov_file_path = f"{in_folder}xov/xov_{arc1}_{arc2}.pkl"

      with open(xov_file_path, "rb") as xov_file: # garbage collector ?
         xov = pickle.load(xov_file)

      contain_nan = xov_table_cvs(xov, writer, xov_time,  distance, derivatives)
      if contain_nan:
         print(f"Found nan in xov_{arc1}_{arc2}.pkl")   
      
    endInit = time.time()
    file.close()


def xov_table_cvs(xov, writer, xov_time,  distance, derivatives):
   import numpy as np
   ...
   for i in range(0,len(tracks_listA)):
      ...
      row = []
      if xov_time:
         tA = xov.xovers['tA'].values[i]
         tB = xov.xovers['tB'].values[i]
           
         row = [int(np.floor(tA))]
         row.append(tA - np.floor(tA))
         row.append(int(np.floor(tB)))
         row.append(tB - np.floor(tB))
...
def xov_header_cvs(file, xov_time,  distance, derivatives):
    columns = ''
    if xov_time:
       columns = ' tA int       |'\
               + ' tA frac               |'\
    ...
    columns = columns + ' Lat (deg)             |'\
                      + ' Lon (deg)             |'\
                      + ' dR                    |'
    if distance:
       columns = columns + ' distA                 |'\
                         + ' distB                 |'
    if derivatives:
       columns = columns + ' dR/dR_A               |'\
                         + ' dR/dA_A               |'\
                         + ' dR/dC_A               |'\
                         + ' dR/dR_B               |'\
                         + ' dR/dA_B               |'\
