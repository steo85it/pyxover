...
if __name__ == '__main__':
    ...
    if data_sim == 'sim':
        if local:
          dirnams = ['../data/SIM_'+epos+'/' + exp + '/' + str(x[1]) + 'res_' + str(x[0]) + 'amp_tst/' for x in cmb]
        else:
          dirnams = ['/att/nobackup/sberton2/MLA/data/SIM_'+epos+'/' + exp + '/' + str(x[1]) + 'res_' + str(x[0]) + 'amp_tst/' for x in cmb]
